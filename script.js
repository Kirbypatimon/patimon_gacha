...
function rollGacha() {
  ...
  for (const rarity in probabilities) {
    ...
    if (roll <= cumulative) {
      ...
      return { text: `${choice} (${rarity})`, rarity };
    }
  }
  // ハズレなしなのでここは通らない想定
...
function updateDisplay() {
  ...
  historyDiv.textContent = history.map((h, i) => `${i+1}. ${h.text}`).join("\n");

  const countsDiv = document.getElementById("counts");
  countsDiv.innerHTML = Object.entries(counts)
    .map(([rarity, num]) => `<b>${rarity}:</b> ${num}`)
    .join(" | ");
}
...
document.getElementById("singleBtn").onclick = () => {
  ...
  document.getElementById("result").textContent = `🎉 ガチャ結果: ${result.text}`;
  updateDisplay();
};
...
document.getElementById("tenBtn").onclick = () => {
  ...
  for(let i=0; i<10; i++){
    const res = rollGacha();
    history.push(res);
    counts[res.rarity] = (counts[res.rarity] || 0) + 1;
    results.push(`${i+1}. ${res.text}`);
  }
